...
public class Game {
    ...
    public void placePiecesForMCTS(Player player , int counter) {
        ...
        for (Piece piece : player.getAvailablePieces()) {
            if (piece.getRank() == 11 && canPlaceAtPosition(positionFlagPlayer1, board, placingBordersPlayer1)) {
                placePiece(piece, positionFlagPlayer1, player);
                pieces.add(piece);
        ...
        //for the marshal
        for (Piece piece : player.getAvailablePieces()) {
            if(piece.getRank() == 10 && canPlaceAtPosition(positionMarshalPlayer1, board, placingBordersPlayer1)){
                placePiece(piece, positionMarshalPlayer1, player);
                pieces.add(piece);
                player.getAvailablePieces().remove(piece);
                break;
            }
        ...
        //major
        for (Piece piece :player.getAvailablePieces()) {
           if(piece.getRank() == 7 && canPlaceAtPosition(positionMajor1Player1, board, placingBordersPlayer1)){
               
               placePiece(piece, positionMajor1Player1, player);
               pieces.add(piece);
               player.getAvailablePieces().remove(piece);
        ...
        //major
        for (Piece piece : player.getAvailablePieces()) {
           if(piece.getRank() == 7 && canPlaceAtPosition(positionMajor2Player1, board, placingBordersPlayer1)){
               
               placePiece(piece, positionMajor2Player1, player);
               pieces.add(piece);
               player.getAvailablePieces().remove(piece);
        ...
        //major
        for (Piece piece : player.getAvailablePieces()) {
           if(piece.getRank() == 7 && canPlaceAtPosition(positionMajor3Player1, board, placingBordersPlayer1)){
               
               placePiece(piece, positionMajor3Player1, player);
               pieces.add(piece);
               player.getAvailablePieces().remove(piece);
        ...
        // the general
        for (Piece piece : player.getAvailablePieces()) {
            if(piece.getRank() == 9 && canPlaceAtPosition(positionGenralPlayer1, board, placingBordersPlayer1)){
                placePiece(piece, positionGenralPlayer1, player);
                pieces.add(piece);
                player.getAvailablePieces().remove(piece);
                break;
            }
        ...
        // the spy
        for (Piece piece : player.getAvailablePieces()) {
            if(piece.getRank() == 1 && canPlaceAtPosition(positionSpyPlayer1, board, placingBordersPlayer1)){
               System.out.println("ahhhh");
                placePiece(piece, positionSpyPlayer1, player);
                pieces.add(piece);
                player.getAvailablePieces().remove(piece);
                break;
           }
        ...
        //bombs
        for (Piece piece : player.getAvailablePieces()) {
           if(piece.getRank()==12 && canPlaceAtPosition(positionBombPlayer1Y, board, placingBordersPlayer1)  ){
               if(counterBomb==0){
                   placePiece(piece, positionBombPlayer1Y, player);
                   pieces.add(piece);
                   player.getAvailablePieces().remove(piece);
                   counterBomb++;
           ...
           if(piece.getRank()==12 && canPlaceAtPosition(positionBombPlayer1X, board, placingBordersPlayer1)){
               if(counterBomb==1){
                   placePiece(piece, positionBombPlayer1X, player);
                   pieces.add(piece);
                   player.getAvailablePieces().remove(piece);
                   counterBomb++;
           ...
           if(piece.getRank()==12 && canPlaceAtPosition(positionBombPlayer1XY, board, placingBordersPlayer1)){
               if(counterBomb==2){
                   placePiece(piece, positionBombPlayer1XY, player);
                   pieces.add(piece);
                   player.getAvailablePieces().remove(piece);
                   counterBomb++;
                   break;
        ...
        for (Piece piece : player.getAvailablePieces()) {
           if(piece.getRank()==8 && canPlaceAtPosition(positionColonel1Player1, board, placingBordersPlayer1)){
               
               if(counterColonel == 0){
                   placePiece(piece, positionColonel1Player1, player);
                   pieces.add(piece);
                   player.getAvailablePieces().remove(piece);
                   counterColonel++;
                   break;
        ...
        for (Piece piece : player.getAvailablePieces()) {
           ...
           if(piece.getRank() == 8 && canPlaceAtPosition(positionColonel2Player1, board, placingBordersPlayer1)){
               
               if(counterColonel == 1){
                   
                   placePiece(piece, positionColonel2Player1, player);
                   pieces.add(piece);
                   player.getAvailablePieces().remove(piece);
                   counterColonel++;
                   break;
